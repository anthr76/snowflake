{ pkgs, ... }: {
  ...
  services.darkman = {
    darkModeScripts = {
      kde-global = ''
        ${pkgs.libsForQt5.plasma-workspace}/bin/lookandfeeltool -platform offscreen --apply "org.kde.breezedark.desktop"
      '';
      kde-gtk-theme = ''
    ...
    lightModeScripts = {
      kde-global = ''
        ${pkgs.libsForQt5.plasma-workspace}/bin/lookandfeeltool -platform offscreen --apply "org.kde.breeze.desktop"
      '';
      kde-gtk-theme = ''
        ${pkgs.dbus}/bin/dbus-send --session --dest=org.kde.GtkConfig \
