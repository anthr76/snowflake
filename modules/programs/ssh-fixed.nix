...
in {
  ...
  config = {
    ...
    programs.ssh-fixed.setXAuthLocation = mkDefault
      (config.services.xserver.enable || config.programs.ssh-fixed.forwardX11
        || config.services.openssh.forwardX11);
    ...
    systemd.user.services.ssh-agent = mkIf cfg.startAgent {
      ...
      serviceConfig = {
        ...
        ExecStart = "${cfg.package}/bin/ssh-agent "
          ...
          ("-P ${cfg.agentPKCS11Whitelist} ") + "-a $SSH_AUTH_SOCK";
        StandardOutput = "null";
        Type = "forking";
        Restart = "on-failure";
